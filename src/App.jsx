...
// يتم استيراد أيقونات Lucide React
...
/**
 * شاشة انتظار التحقق من البريد الإلكتروني
 */
const VerificationPrompt = ({ auth }) => {
    const [loading, setLoading] = useState(false);
    const [message, setMessage] = useState('يرجى التحقق من بريدك الإلكتروني والضغط على رابط التفعيل لإكمال التسجيل.');
    ...
    const handleReload = async () => {
        ...
        try {
            // إجبار إعادة تحميل بيانات المستخدم من Firebase
            await reload(user); 
            // بعد إعادة التحميل، نحصل على الكائن المحدث
            const updatedUser = auth.currentUser; 
            
            if (updatedUser?.emailVerified) {
                // إذا تم التحقق، نقوم بإعادة تحميل الصفحة لإعادة تشغيل منطق App والانتقال إلى لوحة التحكم
...
/**
 * شاشة المصادقة (تسجيل الدخول / التسجيل) - تمثل شاشة الهبوط الرئيسية
 */
    ...
    const handleAuthenticate = async (e) => {
        ...
        try {
            if (view === 'register') {
                ...
                // 3. حفظ الدور في Firestore (لتحديد دور المستخدم ومنع التسجيل مرة أخرى)
                ...
                // 4. تحديث حالة التطبيق لانتظار التحقق
                onAuthSuccess(user); 
            ...
            if (err.code === 'auth/email-already-in-use') {
                errorMessage = 'البريد الإلكتروني مُسجل بالفعل.';
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen p-4 bg-gray-100" dir="rtl">
            <div className="bg-white p-8 md:p-10 rounded-xl shadow-2xl w-full max-w-lg border-t-8 border-teal-600">
                {/* --- الترحيب والهدف (الخاصية المطلوبة) --- */}
                ...
                <h2 className="text-3xl font-bold text-center mb-6 text-teal-800 flex items-center justify-center">
                ...
                <form onSubmit={handleAuthenticate} className="space-y-4">
                    ...
                    >
                        {loading ? (
                            <>
                                <svg className="animate-spin h-5 w-5 ml-3 text-white" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                                جارٍ {isRegisterView ? 'التسجيل...' : 'الدخول...'}
                            </>
                        ) : (
                            isRegisterView ? 'تسجيل الحساب وإرسال بريد التحقق' : 'تسجيل الدخول'
                        )}
                    </button>
                </form>
...
const Dashboard = ({ auth, userRole, userId }) => {
    ...
    // تعريف بطاقات الملاحظات الثمانية
    const cards = [
    ...
    // تصفية البطاقات بناءً على دور المستخدم - (تم إلغاء التصفية حالياً لعرض كل البطاقات كمثال مبدئي)
        ...
        <div className="min-h-screen bg-gray-50" dir="rtl">
                ...
                <div className="max-w-7xl mx-auto flex justify-between items-center">
                    ...
                    <div className="text-left text-sm flex items-center space-x-4 space-x-reverse">
                        <div className="hidden sm:block">
                            ...
                            <p className="text-gray-200 truncate max-w-[200px]" title={userId}>معرف المستخدم: <span className="font-mono">{userId.substring(0, 8)}...</span></p>
                        </div>
                        <button 
                            onClick={handleSignOut}
            ...
            <main className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
                <div className="bg-white p-6 rounded-xl shadow-lg mb-8 border-r-4 border-teal-500">
                    ...
                    <p className="text-gray-600 mt-2">يمكنك الوصول إلى مهام المتابعة الخاصة بك من خلال البطاقات أدناه. دورك الحالي هو: <span className="font-extrabold text-teal-600">{userRole}</span>.</p>
                </div>
                
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredCards.map((card, index) => (
                        <a key={index} href={card.href} className="bg-white p-6 rounded-xl shadow-xl border-t-4 border-teal-600 hover:shadow-2xl hover:border-b-4 transition duration-300 flex flex-col items-start space-y-3">
...
const App = () => {
        ...
        try {
            ...
            const unsubscribe = onAuthStateChanged(authInstance, (user) => {
                if (user) {
                    ...
                    // يجب أن نعتمد على آخر حالة للتحقق من البريد
    ...
    // 2. التحقق من دور المستخدم في Firestore
    useEffect(() => {
        // لا نحتاج للتحقق من الدور إلا إذا كان البريد مفعلًا والمستخدم موجود
        ...
        const checkUserRole = async () => {
            try {
                // مسار خاص بالـ userId المحمي بقواعد الأمان
                ...
                if (docSnap.exists()) {
                    ...
                    // إذا كان المستخدم مفعلًا ولكن لا يوجد دور، هذا يعني مشكلة في التسجيل،
                    // لكن لغرض هذا التطبيق، نفترض أن الدور يجب أن يكون موجودًا بعد التحقق.
                    console.warn("User verified, but role not found in Firestore. Navigating to auth screen for role selection/retry.");
                    
                    // نقوم بتسجيل الخروج لإجبار المستخدم على الدخول مرة أخرى واختيار الدور في شاشة التسجيل
                    // لكن لتبسيط المنطق مؤقتا، سنكتفي بالتحذير وانتظار تحميل الدور.
                    setUserRole(null); 
                }
            } catch (e) {
    ...
    // دالة يتم استدعاؤها بعد نجاح التسجيل أو تسجيل الدخول
    const handleAuthSuccess = useCallback((user) => {
        setUserId(user.uid);
        setIsAuthenticated(true);
        setIsEmailVerified(user.emailVerified);
        // عند نجاح التسجيل، سيتم تعيين isEmailVerified كـ false، وسيعرض المكون
        // VerificationPrompt
    }, []);
    
    // --- العرض المشروط (Conditional Rendering) ---
    
    if (error) {
        return <div className="p-10 text-center bg-red-50 text-red-800 rounded-lg m-4" dir="rtl">خطأ في النظام: {error}</div>;
    }
    ...
    if (!isAuthenticated && auth && db) {
        // 1. المستخدم غير مسجل الدخول: نعرض شاشة التسجيل/الدخول (شاشة الهبوط)
        return <AuthScreen auth={auth} db={db} onAuthSuccess={handleAuthSuccess} />;
    }
    
    if (isAuthenticated && !isEmailVerified && auth) {
        // 2. المستخدم مسجل الدخول لكن البريد غير مفعل: نعرض شاشة التحقق
        return <VerificationPrompt auth={auth} />;
    }
    
    if (userRole) {
        // 3. المستخدم مسجل الدخول، البريد مفعل، والدور محدد: نعرض لوحة التحكم
        return <Dashboard auth={auth} userRole={userRole} userId={userId} />;
    }

    // fallback: إذا كان isAuthenticated و isEmailVerified لكن الدور لم يتم تحميله بعد
